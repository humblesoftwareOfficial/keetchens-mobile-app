...
import { CARD_MEDIA_LIST_STYLE } from "../../styling/cards";
    ...
    () =>
      data?.map((media, index) => (
        <TouchableOpacity
          key={index}
          style={[
            CARD_MEDIA_LIST_STYLE.item_media,
            {
  ...
  return (
    <>
      ...
      {Boolean(data?.length > 1) && (
        <View style={CARD_MEDIA_LIST_STYLE.footer_media}>
          <View style={CARD_MEDIA_LIST_STYLE.container_footer_medias}>
...
export default React.memo(MediasVerticalList);
